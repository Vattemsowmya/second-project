	...
	<div style="margin-top: 20px">
		<c:if test="${!empty forumList}">
			...
			<div style="background-color: lightgrey">
					<p style="padding: 5px; font-size: large;">
						<strong>${forum.forumName}</strong>
					</p>
				</div>
				<div style="background-color: lightgrey">
					<p style="padding: 5px; font-size: medium">${forum.forumDesc}</p>
				</div>
			</c:forEach>
		</c:if>
	</div>
	...
	<sec:authorize access="isAuthenticated()">
		<div class="row">
			<form role="form" data-ng-submit="Comment()">
				...
				<div class=" col-sm-2">
					<button type="submit" data-ng-click="Comment()" class="btn btn-success">Comment</button>
				</div>
			</form>
		</div>
	</sec:authorize>
